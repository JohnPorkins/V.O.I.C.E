...
def list_fact_texts(conn: Optional[sqlite3.Connection] = None) -> list[str]:
    """Все уникальные тексты фактов из fact_embeddings."""
    own = conn is None
    if own:
        conn = sqlite3.connect(str(EMBEDDING_DB_PATH))
        init_embedding_db(conn)
    rows = conn.execute(
        f"SELECT DISTINCT fact_text FROM {FACT_TABLE} ORDER BY created_at DESC"
    ).fetchall()
    if own:
        conn.close()
...
def facts_for_query(query: str, limit: int = 20) -> list[str]:
    """
    Факты, релевантные запросу: по ключевым словам в fact_text.
    Без query — последние limit фактов.
    """
    all_facts = list_fact_texts()
    if not all_facts:
        return []
    q = (query or "").strip()
    if not q:
    ...
    keywords = [
        w
        for w in re.split(r"[^\w\u0400-\u04FF]+", q.lower(), flags=re.UNICODE)
        if len(w) >= 3
    ]
    if not keywords:
        return all_facts[:limit]
...
def build_facts_instructions_block(query: str = "", limit: int = 20) -> str:
    """Текст для session.instructions Realtime API."""
    picked = facts_for_query(query, limit=limit)
    if not picked:
        return ""
    lines = "\n".join(f"- {f}" for f in picked)
    return (
        "Відомі факти з пам'яті системи (використовуй у відповідях, якщо доречно; "
        "згадуй природно, українською). Якщо питають «хто такий X» — відповідай, "
        "спираючись на ці факти, наприклад: «Я знаю, що …»:\n"
        f"{lines}"
    )
